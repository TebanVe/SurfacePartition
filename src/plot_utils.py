...
def plot_contours_on_ring(
    ...
    title: str = "Ring Partitions - Contours",
    figsize: Tuple[int, int] = (10, 8),
    ax: Optional[plt.Axes] = None,
):
    """
    Overlay extracted contour segments on the ring mesh in 2D.
    """
    created_fig = False
    if ax is None:
        fig, ax = plt.subplots(figsize=figsize)
    ...
    if show_mesh:
        ax.triplot(triangulation, 'k-', linewidth=0.4, alpha=0.5)
    if show_vertices:
        ax.plot(vertices[:, 0], vertices[:, 1], 'ko', markersize=1.5, alpha=0.6)

    default_colors = ['red', 'blue', 'green', 'orange', 'purple', 'cyan', 'magenta', 'yellow']
    for region_idx, segments in contours.items():
        color = (colors[region_idx] if colors and region_idx < len(colors)
                 else default_colors[region_idx % len(default_colors)])
        for seg in segments:
            ax.plot(seg[:, 0], seg[:, 1], color=color, linewidth=2.0, alpha=0.9)
...
def plot_partition_fill_from_labels(
    vertices: np.ndarray,
    faces: np.ndarray,
    ...
    title: str = "Ring Partitions - Filled",
    figsize: Tuple[int, int] = (10, 8),
    ax: Optional[plt.Axes] = None,
):
    """
    Fill triangles by region label for a clean partition visualization.
    """
    created_fig = False
    if ax is None:
        fig, ax = plt.subplots(figsize=figsize)
    ...
    triangulation = tri.Triangulation(vertices[:, 0], vertices[:, 1], faces)
    tpc = ax.tripcolor(triangulation, facecolors=triangle_labels, cmap=cmap, edgecolors='k' if show_edges else 'none', alpha=alpha)
    ax.set_aspect('equal')
    ax.set_title(title)
    ax.set_xlabel('x')
    ax.set_ylabel('y')
...
def plot_partitions_with_contours(
    vertices: np.ndarray,
    faces: np.ndarray,
    triangle_labels: np.ndarray,
    contours: Dict[int, List[np.ndarray]],
    title: str = "Ring Partitions",
    figsize: Tuple[int, int] = (12, 10),
    show_edges: bool = True,
):
    """
    Composite plot: filled partitions + contour overlays.
    """
    fig, ax = plt.subplots(figsize=figsize)
    plot_partition_fill_from_labels(vertices, faces, triangle_labels, ax=ax, show_edges=show_edges,
                                    title=title)
    plot_contours_on_ring(vertices, faces, contours, ax=ax, show_mesh=False, show_vertices=False)
    plt.tight_layout()
    return fig, ax 


def plot_vertex_field(
    vertices: np.ndarray,
    faces: np.ndarray,
    values: np.ndarray,
    cmap: str = 'seismic',
    shading: str = 'gouraud',
    title: str = 'Vertex Field',
    figsize: Tuple[int, int] = (10, 8),
    vmin: float = None,
    vmax: float = None,
    ax: Optional[plt.Axes] = None,
    ...
    """
    Plot a scalar field defined at vertices using Matplotlib triangulation.
    """
    created_fig = False
    if ax is None:
        fig, ax = plt.subplots(figsize=figsize)
    ...
    triangulation = tri.Triangulation(vertices[:, 0], vertices[:, 1], faces)
    tpc = ax.tripcolor(triangulation, values, shading=shading, cmap=cmap, vmin=vmin, vmax=vmax)
    ax.set_aspect('equal')
    ax.set_title(title)
    ax.set_xlabel('x')
    ax.set_ylabel('y')
...
def overlay_zero_isoline(
    ...
    """
    Overlay the zero-level isoline of a vertex field on an existing Axes.
    """
    triangulation = tri.Triangulation(vertices[:, 0], vertices[:, 1], faces)
    ax.tricontour(triangulation, values, levels=[0.0], colors=[color], linewidths=linewidth)
    return ax
...
def plot_histogram(
    ...
    """
    Plot a histogram of provided values.
    """
    created_fig = False
    if ax is None:
        fig, ax = plt.subplots(figsize=figsize)
